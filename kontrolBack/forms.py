...
class AddControlForm(forms.ModelForm):
    ...
    class Meta:
        model = Control
        fields = ['name', 'project', 'status', 'controlling', 'date_start', 'date_end', 'checklist']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.user = kwargs.pop('user', None) # do poprawy, podobno niebezpieczne
        super().__init__(*args, **kwargs)
        if self.user:
            user_institution = Institution.objects.filter(pk=self.user.institutionemployee.institution.pk)
